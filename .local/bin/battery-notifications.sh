...
if [ "$CURRENT_PERCENTAGE" -lt $THRESHOLD -a $STATUS = $NOT_CHARGING ]; then
    notify-send "Low battery warning" "Current capacity at $CURRENT_PERCENTAGE%" -h string:synchronous:battery_notif -h string:x-canonical-private-synchronous:battery_notif
fi
